...
// Удаляет ИБ на сервере
// 
// Параметры:
//   ИдентификаторКластера - Строка - внутренний идентификатор кластера серверов.
//   ПараметрыАдминистрированияКластера - см. АдминистрированиеКластера.ПараметрыАдминистрированияКластера
//   ПараметрыАдминистрированияИБ - см. АдминистрированиеКластера.ПараметрыАдминистрированияИнформационнойБазыКластера
//   Режим - Число - определяет, что делать с базой данных
//                   0 - удаляем
//                   1 - оставляем без изменений (режим по умолчанию)
//                   2 - очищаем
Процедура УдалитьИнформационнуюБазу(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Режим = 1) Экспорт
	...
	Команда = "infobase drop --infobase=%1 --infobase-user=%2 --infobase-pwd=%3 %4 " + ПараметрыКластера(ПараметрыАдминистрированияКластера, ИдентификаторКластера);
	РежимУдаления = "";
	Если Режим = 0 Тогда
		РежимУдаления = "--drop-database";
	ИначеЕсли Режим = 2 Тогда
		РежимУдаления = "--clear-database";
	КонецЕсли;
...
// Создает ИБ на сервере
// 
// Параметры:
//   ИдентификаторКластера - Строка - внутренний идентификатор кластера серверов.
//   ПараметрыАдминистрированияКластера - см. АдминистрированиеКластера.ПараметрыАдминистрированияКластера
//   ЗначенияСвойств - Структура - 
//   Режим - Число - определяет, необходимость создания базы данных на сервере SQL
//                   0 - не создаем
//                   1 - создаем (режим по умолчанию)
Функция СоздатьИнформационнуюБазу(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ЗначенияСвойств, Знач Режим = 1) Экспорт
	
	ПараметрыКластера = ПараметрыКластера(ПараметрыАдминистрированияКластера, ИдентификаторКластера);
	
	Команда = "infobase create --locale=ru --license-distribution=allow";
	Если Режим = 1 Тогда
		Команда = Команда + " --create-database";
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из СловарьСвойствНовойИнформационнойБазы() Цикл
	...
	ПоддерживаемыеСвойства = Новый Соответствие;
	ПоддерживаемыеСвойства.Вставить("name",Истина);
	ПоддерживаемыеСвойства.Вставить("dbms",Истина);
	ПоддерживаемыеСвойства.Вставить("db-server",Истина);
	ПоддерживаемыеСвойства.Вставить("db-user",Истина);
	ПоддерживаемыеСвойства.Вставить("db-pwd",Истина);
	ПоддерживаемыеСвойства.Вставить("db-name",Истина);
	ПоддерживаемыеСвойства.Вставить("scheduled-jobs-deny",Истина);
	ПоддерживаемыеСвойства.Вставить("date-offset",Истина);
	
	ДобавитьПараметрыКомандыПоСловарю(Команда, СловарьСвойствНовойИнформационнойБазы(), НовыеЗначенияСвойств, ПоддерживаемыеСвойства);
	
	Результат = ЗапуститьКоманду(Команда + " " + ПараметрыКластера, ПараметрыАдминистрированияКластера);
...
Функция СвойстваИнформационнойБазы(Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИБ, Знач Словарь) Экспорт
...
Функция СловарьСвойствНовойИнформационнойБазы()
	
	Результат = Новый Структура();
	Результат.Вставить("Имя");
	Результат.Вставить("ТипСервераБД");
	Результат.Вставить("ИмяСервераБД");
	Результат.Вставить("ПользовательБД");
	Результат.Вставить("ПарольБД");
	Результат.Вставить("ИмяБазыНаСервере");
	Результат.Вставить("БлокировкаРегламентныхЗаданий");
	Результат.Вставить("СмещениеДат");
	
	Результат.Имя = "name";
	Результат.ТипСервераБД = "dbms";
	Результат.ИмяСервераБД = "db-server";
	Результат.ПользовательБД = "db-user";
	Результат.ПарольБД = "db-pwd";
	Результат.ИмяБазыНаСервере = "db-name";
	Результат.БлокировкаРегламентныхЗаданий = "scheduled-jobs-deny";
	Результат.СмещениеДат = "date-offset";
