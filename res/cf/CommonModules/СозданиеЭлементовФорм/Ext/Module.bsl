...
//Быстрая функция по созданию декорации гиперссылки
//
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - Форма на которой создаются элементы
//  ИмяГиперссылки - Строка - Имя элемента на форме
//	РодительДекорации	- Строка - достаточно указать название элемента формы
//					- Элементы - можно в качестве родителя передать элемент формы
//	Заголовок - Строка - Заголовок гиперссылки
//  ИмяОбработчикаНажатие - Строка
//	ЭлементПеред	- Строка - достаточно указать название элемента формы перед которым требуется вставить новый элемент
//					- Элементы - можно передать элемент формы
//
Функция СздДекорацияГиперссылка(Форма,
	ИмяГиперссылки,
	РодительДекорации,
	Заголовок,
	ИмяОбработчикаНажатие,
	ЭлементПеред = Неопределено) Экспорт

	СвойстваГиперссылки = Новый Структура();
	СвойстваГиперссылки.Вставить("Гиперссылка", Истина);
	СвойстваГиперссылки.Вставить("Заголовок", Заголовок);
	СобытияГиперссылки = Новый Структура();
	СобытияГиперссылки.Вставить("Нажатие", ИмяОбработчикаНажатие);
	НовыйЭлемент = СздДекорация(Форма, ИмяГиперссылки, РодительДекорации, СвойстваГиперссылки, СобытияГиперссылки, ЭлементПеред);
	
	Возврат НовыйЭлемент;

КонецФункции // СздДекорацияГиперссылка()
