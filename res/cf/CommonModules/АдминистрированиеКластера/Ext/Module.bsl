...
// Возвращает структуру для создание новой информационной базы с помощью RAC.
// Примечание: для COM не используется.
Функция ПараметрыНовойИнформационнойБазы() Экспорт
	
	ПараметрыИБ = Новый Структура;
	ПараметрыИБ.Вставить("Имя", "");
	ПараметрыИБ.Вставить("ТипСервераБД", "MSSQLServer");
	ПараметрыИБ.Вставить("ИмяСервераБД", "");
	ПараметрыИБ.Вставить("ПользовательБД", "");
	ПараметрыИБ.Вставить("ПарольБД", "");
	ПараметрыИБ.Вставить("ИмяБазыНаСервере", "");
	ПараметрыИБ.Вставить("БлокировкаРегламентныхЗаданий", Истина);
